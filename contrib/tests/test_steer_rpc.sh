...
"
            ...
            if echo "$B_OUTPUT" | grep -q "agent-a\|Hello Agent B\|e2e-test-001\|peer_message\|📨"; then
...
"
                ...
                if echo "$A_OUTPUT" | grep -q "agent-b\|Got your message\|e2e-reply-001\|peer_message\|📨"; then
...
"
        ...
        for i in $(seq 1 15); do
            ...
            # Check for signs the agent woke up: the marker text, or peer_message\|📨 notification,
            # or any new agent output (cost changed, token counts appeared)
            if echo "$A_AFTER" | grep -q "$WAKEUP_MARKER\|wakeup-sender\|WAKEUP_TEST\|peer_message\|📨"; then
